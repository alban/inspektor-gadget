...
func (i *wasmOperatorInstance) addFieldFuncs(env wazero.HostModuleBuilder) {
	...
	exportFunction(env, "fieldCopy", i.fieldCopy,
		[]wapi.ValueType{
			wapi.ValueTypeI32, // Accessor
			wapi.ValueTypeI32, // Data
			wapi.ValueTypeI32, // Kind
...
// fieldCopy returns the field's value.
// Params:
// - stack[0]: Field handle
// - stack[1]: Data handle
// - stack[2]: Kind
...
func (i *wasmOperatorInstance) fieldCopy(ctx context.Context, m wapi.Module, stack []uint64) {
	fieldHandle := wapi.DecodeU32(stack[0])
	dataHandle := wapi.DecodeU32(stack[1])
	fieldKind := api.Kind(wapi.DecodeU32(stack[2]))
	fieldDst := stack[3]
	...
	if err != nil {
		i.logger.Warnf("fieldCopy for field %q failed: %v", field.Name(), err)
		stack[0] = 0
		return
	}
