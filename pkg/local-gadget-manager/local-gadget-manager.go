...
func (l *LocalGadgetManager) AddTracer(gadget, name, containerFilter string) error {
	...
	trace := factory.LookupOrCreate(types.NamespacedName{Namespace: "gadget", Name: name})
...
func (l *LocalGadgetManager) Operation(name, opname string) error {
	...
	if opname != "" {
		tracer.trace.Operation(tracer.traceResource, opname, nil)
	}
...
func (l *LocalGadgetManager) Delete(name string) error {
	...
	err := factory.Delete(types.NamespacedName{Namespace: "gadget", Name: name})
	if err != nil {
		return err
	}
...
func NewManager() (*LocalGadgetManager, error) {
	...
	l.traceFactories["biolatency"] = &biolatency.TraceFactory{}
	l.traceFactories["dns"] = &dns.TraceFactory{}
	l.traceFactories["network-policy-advisor"] = &networkpolicyadvisor.TraceFactory{}
	l.traceFactories["process-collector"] = &processcollector.TraceFactory{}
	l.traceFactories["seccomp"] = &seccomp.TraceFactory{}
	l.traceFactories["socket-collector"] = &socketcollector.TraceFactory{}
