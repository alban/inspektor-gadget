...
func (t *Tracer[Event]) searchForLibrary(root string) ([]string, error) {
	var unsecurerTargetPaths []string

	filePath := t.attachFilePath
	if !filepath.IsAbs(filePath) {
		ldCachePath := "/etc/ld.so.cache"
		ldCachePaths, err := parseLdCache(root, ldCachePath, filePath)
		if err != nil {
...
func (t *Tracer[Event]) attach(containerPid uint32) {
	...
	unsecuredAttachFilePaths, err := t.searchForLibrary(root)
	if err != nil {
	...
	for _, filePath := range unsecuredAttachFilePaths {
		...
		file, err := secureopen.SecureOpen(root, filePath)
		if err != nil {
