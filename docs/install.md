...
This option uses a VM driver (like Virtualbox or kvm2). It's compatible with all
the gadgets that don't require BTF support. This option uses a custom minikube
...
The traceloop and network policy advisor gadgets only work with this driver.
...
This option works for tools that require BTF support. You must have BTF support
on the host to be able to use it.
